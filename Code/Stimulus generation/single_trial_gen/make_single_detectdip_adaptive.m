...
filepath = strcat('/Users/cannon/Documents/MATLAB/Entrainment-Contingency/stimulus_sequences/adaptive_dip_', timestamp);
mkdir(filepath);
...
all_volumes = .5*exp(0:-.5:-.5*7);%[.012,.0105,.009,.0075, .006, .0045, .003, .0015];

gen_random_trial = @(filename, valid) stim_maker_detectdip_alldiffs(filename, all_volumes, uniform_rand_init_gen(), 1, uniform_rand_delay_gen(), 0, trial_length_after_cue, valid);
gen_interval_trial = @(filename, valid) stim_maker_detectdip_alldiffs(filename, all_volumes, uniform_rand_init_gen(), 1, mean_delay, 0, trial_length_after_cue, valid);
gen_near_interval_trial = @(filename, valid) stim_maker_detectdip_alldiffs(filename, all_volumes, uniform_rand_init_gen(), 1, mean_delay, perturbation(), trial_length_after_cue, valid);

gen_beat_trial = @(filename, valid) stim_maker_detectdip_alldiffs(filename, all_volumes, uniform_rand_init_gen(), 3, uniform_rand_beat_gen(), 0, trial_length_after_cue, valid)
gen_near_beat_trial = @(filename, valid) stim_maker_detectdip_alldiffs(filename, all_volumes, uniform_rand_init_gen(), 3, uniform_rand_beat_gen(), perturbation(), trial_length_after_cue, valid)
