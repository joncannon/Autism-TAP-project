...
for i = 1:length(all_periods)
    ...
    for j=1:2
        block = stim_maker_standard(strcat(filepath, 'listen_metronome_', all_filewords{i}, '_', datestr(clock, 'dd-mmm-yyyy_HH+MM+SS')), all_periods(i), 25+floor(rand()*5), true, i, params);
    ...
    for j=1:3
        block = stim_maker_standard(strcat(filepath, 'tap_metronome_', all_filewords{i}, '_', datestr(clock, 'dd-mmm-yyyy_HH+MM+SS')), all_periods(i), 25+floor(rand()*5), false, i+10, params);
    ...
    for j = 1:4
        block = stim_maker_deviant(strcat(filepath, 'listen_deviant_metronome_', j, '_', all_filewords{i}, '_', datestr(clock, 'dd-mmm-yyyy_HH+MM+SS')), all_periods(i), 25+floor(rand()*5), 0.3, true, i+20, params);
        block.instruction_type = 'listen';
        block.instructions = rxn_inst;
        listening_blocks{end+1} = block;
    end
...
%     tapping_blocks{end}.params = params;
    ...
    for j = 1:4
        block = stim_maker_omission(strcat(filepath, 'listen_omission_metronome_', j, '_', all_filewords{i}, '_', datestr(clock, 'dd-mmm-yyyy_HH+MM+SS')), all_periods(i), 25+floor(rand()*5), 0.3, true, i+30, params);
        block.instruction_type = 'listen';
        block.instructions = rxn_inst;
        listening_blocks{end+1} = block;
    end
    ...
    for j=1:3
        block = stim_maker_omission(strcat(filepath, 'tap_omission_metronome_', all_filewords{i}, '_', datestr(clock, 'dd-mmm-yyyy_HH+MM+SS')), all_periods(i), 25+floor(rand()*5), 0.3, false, i+40, params);
        block.instruction_type = 'tap';
        block.instructions = tapalong_inst;
        tapping_blocks{end+1} = block;
    end
