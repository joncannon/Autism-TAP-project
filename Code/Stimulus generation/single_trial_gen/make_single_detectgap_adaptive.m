...
all_durations = .012*exp(0:-.35:-.35*7);%[.012,.0105,.009,.0075, .006, .0045, .003, .0015];
...
gen_near_interval_trial = @(filename, valid) stim_maker_detectgap_alldiffs(filename, all_durations, uniform_rand_init_gen(), 1, mean_delay, perturbation(), trial_length_after_cue, valid);
...
gen_near_beat_trial = @(filename, valid) stim_maker_detectgap_alldiffs(filename, all_durations, uniform_rand_init_gen(), 3, uniform_rand_beat_gen(), perturbation(), trial_length_after_cue, valid)
